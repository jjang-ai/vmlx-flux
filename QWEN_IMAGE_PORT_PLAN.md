...
## Reference files
- Template: `Sources/vMLXFluxModels/ZImage/ZImageNative.swift` (proven).
- Flux double-stream block (for MM-DiT): `Sources/vMLXFluxKit/FluxDiT.swift`.
- Shared kit: `FlowMatchScheduler`, `VAE`, `WeightLoader`, `MathOps`, `LatentSpace`.
- API contract + proof bar: `OSAURUS_VMLX_FLUX_INTEGRATION_SPEC.md`.
