...
- **Per-model status:** Osaurus `vmlx-origin/main` has current scanner/load
  ...
  `OSAURUS_IMAGE_OPENAPI.json` is the route/schema contract for the
...
2. Use `OSAURUS_IMAGE_OPENAPI.json` as the route/schema source and
   `OSAURUS_IMAGE_UI_MANIFEST.json` as the live model/control exposure
...
1. Wire the osaurus app/server bridge: `/v1/images/models`,
   ...
   exclusion around the full stream drain. Use `OSAURUS_IMAGE_OPENAPI.json`
