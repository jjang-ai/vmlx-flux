...
VAE conditioning latent proof, but no edited-image output yet.
...
| qwen-image-edit | PARTIAL q4 load + source-image preprocess + prompt-token + Qwen2.5-VL prompt-image encode + VAE conditioning latent proof; q3/q4/q5 scan loadable | — | — | needs transformer edit loop + live edited image |
...
- `Qwen-Image-Edit-mflux` (89GB) with nested `q3`, `q4`, `q5`, `q6` variants.
  ...
  `token_image_count=196`, finite stats). Image editing is still unimplemented
  because conditioning concat through the transformer and denoise/decode are not
  wired.
...
1. **qwen-image-edit:** the txt2img pipeline works, and the q4 prompt-token, Qwen2.5-VL prompt-image encode, and source-image VAE conditioning boundaries are live-proven. Read `/tmp/mflux-ref/src/mflux/models/qwen/variants/edit/` + transformer edit code next. The prompt/image side now matches the mflux edit template (`edit_template_start_idx=64`, `Picture N:` image prefix, `<|vision_start|><|image_pad|><|vision_end|>`) and splices Qwen-VL image features into the text-token stream at `image_token_id=151655`; next wire the conditioning-latent concat through the transformer and denoise/decode loop.
   - Current staged bundle is already present at `~/.mlxstudio/models/image/Qwen-Image-Edit-mflux`; use the `q4` variant first (`Qwen-Image-Edit-mflux-q4`) for implementation/proof. Current q4 proof artifacts: `docs/local/vmlx-flux-probes/2026-06-16-qwen-edit-q4-manifest-load/Qwen-Image-Edit-mflux-q4-load.json` (`load_status=loaded`, `generate_requested=false`), `docs/local/vmlx-flux-probes/2026-06-16-qwen-edit-q4-preprocess-live/Qwen-Image-Edit-mflux-q4-load.json` (`edit_requested=true`, typed `notImplemented` after source-image tensors/patches), `docs/local/vmlx-flux-probes/2026-06-16-qwen-edit-q4-prompt-live/Qwen-Image-Edit-mflux-q4-load.json` (`qwen_edit_prompt_tokens[0].status=tokenized`, `image_token_count=196`, `template_drop_index=64`), `docs/local/vmlx-flux-probes/2026-06-16-qwen-edit-q4-vl-encode-live/Qwen-Image-Edit-mflux-q4-load.json` (`qwen_edit_vision_language[0].status=encoded`, `feature_shape=196x3584`, `prompt_embeds_shape=1x212x3584`, finite stats), and `docs/local/vmlx-flux-probes/2026-06-16-qwen-edit-q4-conditioning-live/Qwen-Image-Edit-mflux-q4-load.json` (`qwen_edit_conditioning.status=encoded`, `latents_shape=1x4096x64`, `image_ids_shape=1x4096x3`, finite stats).
...
- `jjang-ai/vmlx-flux` branch **`native-zimage-proven`** — all native work: `9915417` (z-image vendor+proof), `4a88089` (resolution fix), `a2c1a28` (flux-schnell working), `f82dd1b` (probe flags), `fc6e5b1` (qwen-image working + ideogram scaffold), `f7014e0` (handoff); current HEAD adds qwen-edit nested scan, manifest-gated q4 load, q4 source-image tensor preprocess proof, and q4 VAE conditioning latent proof. Open a PR from this branch to vmlx-flux main when ready.
